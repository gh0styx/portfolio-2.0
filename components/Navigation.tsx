...
export default function Navigation() {
  ...
  useEffect(() => {
    const p = () => {
      const date = new Date();
      setCurrentTime(date.toLocaleTimeString("en-US", { hour: "2-digit", minute: "2-digit", second: "2-digit", hour12: true }));
    }
    p();
  ...
  return (
    <motion.nav
      ...
      <motion.div
        ...
        className={`max-lg:block lg:hidden overflow-hidden bg-background/95 backdrop-blur-xl absolute top-full left-0 right-0 z-60 will-change-transform border-b border-white/10 ${isMobileMenuOpen ? "pointer-events-auto" : "pointer-events-none"
          }`}>
